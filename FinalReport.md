...
As our final stretch, we did not want to waste the price data for the whole year, so we would be using the data for our simple trading strategy. In the end, we would check our daily log returns of the portfolio after 262 days of trading.
...
For each day, we would rank the previous day Sharpe ratio of an individual stock. We would long the 5 stocks that had the highest Sharpe ratios and shorting the 5 with the lowest Sharpe ratios. For the first day, since we did not have the data to calculate the standard deviation, we would be using the log returns as our ranking scheme instead.
